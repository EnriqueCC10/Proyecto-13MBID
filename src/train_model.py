...
def create_preprocessor(features_X):
    ...
    # ---------------------------------------------------------------------------
    num_cols = features_X.select_dtypes(
        include=["int64", "float64"]
    ).columns.tolist()

    cat_cols = features_X.select_dtypes(
...
def train_model(
    ...
    """
    Función principal para entrenamiento del modelo final.
    """
    ...
    mlflow.set_experiment(
        "Proyecto 13MBID - Model Training Pipeline"
    )
    ...
    Path("docs/figures").mkdir(
        parents=True,
        exist_ok=True
    )

    cm_path = "docs/figures/confusion_matrix_randomforest.png"
    ...
    with mlflow.start_run(
        run_name="Pipeline (prod) - RandomForest"
    ):
    ...
    # ---------------------------------------------------------------------------
    Path(model_output_path).parent.mkdir(
        parents=True,
        exist_ok=True
    )

    Path(preprocessor_output_path).parent.mkdir(
        parents=True,
        exist_ok=True
    )

    Path(metrics_output_path).parent.mkdir(
        parents=True,
        exist_ok=True
    )
