...
with st.form("prediction_form"):
    ...
    with col2:
        nivel_educativo = st.selectbox(
            "Nivel Educativo", 
            options=["PRIMARIO", "SECUNDARIO_COMPLETO", "TERCIARIO", "UNIVERSITARIO_INCOMPLETO", "UNIVERSITARIO_COMPLETO", "POSGRADO"])
        situacion_vivienda = st.selectbox(
            "Situación de Vivienda", 
            options=["ALQUILER", "PROPIA", "HIPOTECADA", "OTRA"])
    ...
    with col4:
        ...
        antiguedad_empleado = st.number_input("Antigüedad del Empleado (años)", min_value=0.0, max_value=50.0, value=1.0, step=0.5)

    with col5:
        objetivo_credito = st.selectbox(
            "Objetivo del Crédito", 
            options=["PERSONAL", "VIVIENDA", "VEHICULO", "NEGOCIOS", "EDUCACIÓN", "OTRO"])
        tasa_interes = st.number_input("Tasa de interés (%)", min_value=0.0, value=15.0, step=0.01, format="%.2f")
        pct_ingreso = st.number_input("Porcentaje de Ingreso (%)", min_value=0.0, max_value=1.0, value=0.1, step=0.01, format="%.2f")
    ...
    with col8:
        operaciones_mensuales = st.number_input("Operaciones mensuales", min_value=0.0, value=3.0, step=0.5, format="%.1f")
        operaciones_semanales = st.number_input("Operaciones semanales", min_value=0.0, value=0.8, step=0.01, format="%.4f")

    with col9:
        gasto_promedio_operacion = st.number_input("Gasto promedio por operación", min_value=0.0, value=27.24, step=0.01, format="%.4f")
        estabilidad_laboral = st.number_input(
            "Estabilidad laboral (antigüedad / edad)", 
            min_value=0.0, max_value=1.0, value=0.24, step=0.01, format="%.4f",
            help="Relación entre antigüedad laboral y edad del cliente."
        )
...
if submit_button:
    input_data = {
        ...
        "operaciones_semanales": operaciones_semanales,
