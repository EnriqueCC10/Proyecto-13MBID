...
class PredictionRequest(BaseModel):
    ...
    antiguedad_empleado: float = Field(..., description="Antigüedad laboral del cliente")
    situacion_vivienda: str = Field(..., description="Situación de vivienda")
    ingresos: int = Field(..., description="Ingresos mensuales")
    objetivo_credito: str = Field(..., description="Objetivo del crédito")
    pct_ingreso: float = Field(..., description="Porcentaje del ingreso destinado al crédito")
    tasa_interes: float = Field(..., description="Tasa de interés")
    estado_credito: int = Field(..., description="Estado del crédito")
    antiguedad_cliente: float = Field(..., description="Antigüedad como cliente")
    estado_civil: str = Field(..., description="Estado civil")
    estado_cliente: str = Field(..., description="Estado del cliente")
    gastos_ult_12m: float = Field(..., description="Gastos últimos 12 meses")
    genero: str = Field(..., description="Género")
    limite_credito_tc: float = Field(..., description="Límite de tarjeta de crédito")
    nivel_educativo: str = Field(..., description="Nivel educativo")
    personas_a_cargo: float = Field(..., description="Cantidad de personas a cargo")
    ...
    operaciones_semanales: float = Field(..., description="Operaciones semanales")
    estabilidad_laboral: float = Field(..., description="Relación entre antigüedad laboral y edad")
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "operaciones_semanales": 0.807692,
